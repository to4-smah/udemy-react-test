  ...
  getQuery: function getQuery(store, path, appendNode) {
    ...
    while (path.type === 'client') {
      ...
      if (_node instanceof require('./RelayQuery').Field) {
        ...
        process.env.NODE_ENV !== 'production' ? require('fbjs/lib/warning')(!prevField || prevField !== EDGES || !_node.isConnection(), 'RelayQueryPath.getQuery(): Cannot generate accurate query for ' + 'path with connection `%s`. Consider adding an `id` field to each ' + '`node` to make them refetchable.', schemaName) : void 0;
